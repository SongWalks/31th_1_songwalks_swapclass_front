...
const ExchangeRequestPage = () => {
  ...
  // 1. 공통 상태 관리
  const [activeTabId, setActiveTabId] = useState<string>('sent');
  ...
  const fetchData = async () => {
    try {
      ...
      if (sentRes.success) {
        setSentProposal(sentRes.data || null);
      }
    } catch (error) {
      ...
      {activeTabId === 'received' ? (
        ...
        <div className="px-5 pt-6 flex flex-col gap-4">
          {[1, 2, 3].map((priority) => {
            ...
            const rankProposals = receivedProposals.filter(
              (item) => item.matchRank === priority,
            );
        ...
        <div className="flex-1 flex flex-col px-5 pt-6">
          {!sentProposal ? (
            ...
            /* 2. 보낸 요청 데이터가 존재할 때 */
            <div className="w-full bg-white border border-gray-200 rounded-lg p-5 flex flex-col relative shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <span className="text-black text-lg font-medium leading-5 tracking-wide">
                  {sentProposal.targetCourse}
                </span>
                <Badge
                  variant="lightBlueOutline"
                  className="!text-brand-lightBlue !bg-white"
                >
                  {sentProposal.targetType}
                </Badge>
              </div>

              <div className="flex flex-col gap-1.5 pl-1 mb-5">
                {sentProposal.wantedCourses.map((course, idx) => (
              ...
              <div className="flex items-center text-zinc-400 text-xs font-normal gap-2.5 mb-5">
                <div className="flex items-center gap-1">
                  <Icon icon="lucide:clock" className="w-3.5 h-3.5" />
                  <span>{sentProposal.remainTime}</span>
                </div>
                <span>|</span>
                <div className="flex items-center gap-1 cursor-pointer">
                  <img
                    ...
                    className="w-3.5 h-3.5"
                  />
                  <span>받은 요청 {sentProposal.requestCount}개</span>
