...
const SpecificPostsPage: React.FC = () => {
  ...
  // 💡 제안을 이미 보냈는지 (보냈으면 "교환 요청하기" 버튼이 "교환 철회하기"로 바뀜)
  ...
  // 💡 교환 요청 철회
  // TODO: 실제로는 SelectMyPostPage에서 제안 보낼 때 응답으로 받는 proposalId를 저장해뒀다가
  // 여기서 DELETE /api/proposals/{proposalId} 같은 식으로 호출해야 함 (엔드포인트/스키마 확인 필요)
  const handleWithdrawExchange = async () => {
    if (isWithdrawing) return;
    if (!window.confirm('교환 요청을 철회하시겠습니까?')) return;

    try {
      setIsWithdrawing(true);
      // TODO: 실제 철회 API 연동
      console.warn('교환 요청 철회 API 연동 필요 (proposalId 저장 안 돼있음)');
      setHasProposed(false);
      setToastMessage('교환 요청이 철회되었습니다.');
      setShowToast(true);
    } catch (error) {
      console.error('교환 요청 철회 실패:', error);
      alert('교환 요청 철회 중 오류가 발생했습니다.');
    } finally {
            ...
            {hasProposed ? (
              <>
                <p className="text-center text-gray-500 text-[11px] font-light">
                  철회 후 다른 게시글에 다시 요청을 보낼 수 있습니다.
                </p>
                <button
                  onClick={handleWithdrawExchange}
                  disabled={isWithdrawing}
                  className="w-full h-14 rounded-2xl outline outline-1 outline-offset-[-1px] outline-rose-400 bg-white text-rose-500 text-lg font-semibold tracking-wide hover:bg-rose-50 transition-colors disabled:opacity-60"
                >
                  {isWithdrawing ? '철회 중...' : '교환 철회하기'}
                </button>
              </>
            ) : (
              <>
...
export default SpecificPostsPage;import React, { useCallback, useEffect, useState } from 'react';
import { useLocation, useNavigate, useParams } from 'react-router-dom';
import { Icon } from '@iconify/react';
import Header from '@/components/layout/Header';
import { IconButton } from '@/components/common/IconButton';
